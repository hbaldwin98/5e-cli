...
Use `/v1/embeddings` and `/v1/chat/completions` so OpenRouter and similar proxies work. The embedding cache is keyed by corpus fingerprint, base URL, embed model, and the token limit. A rebuild still carries over every vector from the old cache whose chunk text is unchanged, as long as the embed model matches, and embeds only the rest: the fingerprint identifies the data tree rather than its text, so the same data copied to another machine or folder can fingerprint differently (git HEAD in one place, a size hash in another), and that alone must not cost a full re-embed. Do not store the API key.

**Embedding build progress.** Building or rebuilding the cache reports `ask.EmbedProgress{Done, Total, Phase}` per batch through `Config.OnProgress` when set, instead of only writing a preformatted line to `Config.Progress`; `OnProgress` supersedes `Progress` when both are set. The CLI wires `internal/cli/embedProgressRenderer` for `ask`/`chat` (never for `5e mcp`, whose stderr progress stays line-oriented even on a tty, since it's not meant to be watched interactively): on a TTY it draws a determinate Bubbles v2 progress bar, redrawing the same line and finalizing with a trailing newline once `Done >= Total`; off a TTY it returns `nil` so `Config.Progress`'s existing plain `"embedding 40/120"` line keeps working unchanged. **Cancellation:** `Execute` converts SIGINT into `cmd.Context()` cancellation (a second Ctrl-C still kills the process outright) rather than letting the OS kill the process directly, so an in-flight embedding HTTP request aborts via its request context and `writeCache`'s deferred cleanup removes the `.tmp` file — the real cache path is only ever touched by the final atomic rename on full success, so a cancelled build always leaves the existing cache (or no cache) exactly as usable as it was before the attempt.
...
**Visual language.** `internal/cli/style.go` defines one small Lip Gloss vocabulary — `Heading`, `Muted`, `Error`, `Success` — built by `styles(w)` and applied consistently across `ask`, `chat`, and session management (`writeAskHits`'s kind/name/score/snippet, REPL error lines, and the success confirmations for `clear`/`rm`/`rename`/`export`/`import`). It deliberately does not touch Glamour-rendered answers, which already have their own presentation. Styling is disabled — every style renders as plain, identical-to-input text — whenever `w` is not an interactive terminal (`isTTY`, which already excludes `--json` output and a redirected/piped stdout) or `NO_COLOR` is set, so no code path has to remember to skip styling for a script; it is unconditionally safe to call `styles(w)` and use the result.

**Loading indicators.** `internal/cli/status.go` provides two Bubbles v2 spinner primitives, both no-ops off a TTY (never animate in `--json` mode or a redirected/piped destination) and both driven manually off `spinner.MiniDot`'s frame set rather than a full `tea.Program`, since a plain animation loop that writes `\r\x1b[2K<frame> <label>` needs neither the model/update/view machinery nor its own input handling. `runWithStatus` wraps a whole operation (`ask --retrieve-only` uses it, labeled "retrieving") and clears when the wrapped work returns. `runSpinner` instead runs *alongside* the caller's own work and returns a `stop` func — what streaming needs: `ask`/`chat`'s TTY path starts it labeled "thinking", and the `AskStream`/`chat.AskStream` `onDelta` callback calls `stop()` on its first invocation, so the spinner covers retrieval and the wait for the first token and then gets out of the way once the streamed answer itself starts arriving; `stop()` blocks until the spinner goroutine's last write completes, so the delta write that follows it can never race the spinner's own writes to the same destination.

**Streaming.** `client.ChatMessagesStream` sets `stream: true` and reads Server-Sent Events off the response body (`data: {...}` lines, terminated by `data: [DONE]`), calling `onDelta` with each token as it arrives and still returning the complete answer, which `ask.AskStream` / `ask.ConverseStream` / `chat.AskStream` validate citations against exactly as the non-streaming path does — streaming only changes when the text becomes visible, never what grounds it. The CLI (`isTTY`, `internal/cli/render.go`) streams raw deltas straight to stdout only on an interactive terminal in human mode; `--json` needs one complete object to encode, and a redirected or piped stdout gets one deterministic write a script can rely on. A streamed answer is not re-rendered as Markdown afterward — the raw deltas already reached the screen — so streaming and Glamour rendering are mutually exclusive per turn, not layered.

**Surfacing API errors.** A non-2xx HTTP response's body is an OpenAI-compatible `{"error":{"message":"..."}}` document, not plain text; `errorBodyMessage` (in both `internal/ask/client.go` and `internal/provider/models.go`, duplicated rather than shared since `provider` can't import `ask` without cycling back) tries to parse it and surface just `.error.message`, falling back to the raw body (truncated) only when it isn't that shape. Every non-2xx response used to have its raw JSON body dumped straight into the error text, which meant an escaped quote inside the API's own message (e.g. `The model \"x\" does not exist`) showed up to the user as a literal backslash instead of the message OpenAI/OpenRouter actually meant to say.
...
A session holds three things: the transcript, the notes the user recorded, and an optional adventure scope. That scope *adds* the module's prose to the rules corpus; `--adventure-only` or `/adventure <id-or-title> only` is the narrower reading, and it is stored on the session because a conversation that excludes the rulebooks should keep excluding them. `/adventure` accepts either the 5etools id (`LMoP`) or the catalog title (`Lost Mine of Phandelver`), same as `--adventure`; the trailing `only` is matched against the last word of the input, not the first space, so a multiword title like `/adventure Lost Mine of Phandelver only` still parses. Retrieval knobs (`--edition`, `--kind`, `--source`, `--limit`) are per invocation, so a saved conversation never carries a filter from a previous run.
...
**Corrupt sessions.** `Store.List` still never lets one unparseable session file hide the rest of the list, but it no longer swallows the failure either: it returns the file name and error alongside the sessions that did parse, and `chat list` prints one line per corrupt file to stderr. `--json` output is unchanged (still the bare session array), so the warning does not have to be filtered out of a script's JSON.
...
It does not reimplement chat's domain logic: every slash command runs through the same `chatCommand` the plain REPL uses (its stdout/stderr captured into a scratch `*cobra.Command` and folded into the transcript, so behavior — including which turns get persisted — stays identical between the two REPL paths), and every question runs through `chat.AskStream`, streamed the same way the plain TTY path streams. A textarea takes multiline input (Enter sends; Ctrl-J/Alt-Enter inserts a literal newline), a viewport holds the scrollback, and Up/Down recall submitted lines (questions and slash commands alike) the way shell history does, gated on the input having no newline of its own so it doesn't fight cursor movement inside a multiline draft. Esc cancels only the in-flight turn's context (leaving the session and transcript alone) and never quits; Ctrl-C (or Ctrl-D) quits, cancelling any in-flight turn on the way out. `WithoutSignalHandler` is set so Ctrl-C arrives through the normal raw-mode key-event path and runs that cleanup rather than Bubble Tea's own SIGINT force-quit. The textarea uses the terminal's real cursor (`SetVirtualCursor(false)`, placed through `tea.View.Cursor`) rather than a virtual one: a virtual cursor re-renders a reverse-video cell mid-line on every Left/Right, and the renderer's diff of that edit can use insert/delete-character sequences that some terminals and multiplexers apply wrongly, smearing the input line. A Bubbles v2 spinner shows "thinking" while a turn has been submitted but no token has arrived yet, ticking only while `streaming` is true so it stops on its own once the first delta (or an error, or a cancellation) arrives. The transcript is word-wrapped to the viewport's width (`wrapToWidth`, via a `lipgloss.NewStyle().Width(...)` render) before every `SetContent`, since Bubbles v2's viewport only scrolls — it never wraps a long line on its own — so an answer wider than the terminal would otherwise run off the right edge instead of flowing to the next line.

A finished answer is rendered as Markdown (`renderAnswer`, via a width-bound `glamour.TermRenderer`) before it joins the transcript — the source text routinely carries bold/italics/tables/headers, and without this it showed up as literal `**`/`#`/table-pipe syntax. Streamed deltas are deliberately left raw while a turn is in flight (a partial Markdown document renders unpredictably mid-stream); only the complete answer gets rendered, once, in `finishTurn`.

The transcript is a slice of `transcriptBlock`s, not one flat string: most blocks are plain text, word-wrapped to the viewport's width on every `refreshViewport` (`wrapToWidth`, via a `lipgloss.NewStyle().Width(...)` render, since Bubbles v2's viewport only scrolls — it never wraps a line on its own); a glamour-rendered answer is stored `preWrapped` and never gets that treatment a second time, since re-running a word-wrap over already-wrapped, already-ANSI-styled text is exactly the kind of thing that quietly mangles it (a naive single-string transcript would do this on every render).

Scrolling the transcript is always PgUp/PgDn, or ctrl+u/ctrl+f (`ScrollUp`/`ScrollDown` in `chatKeyMap`, calling the viewport's own `PageUp`/`PageDown`; ctrl+b is deliberately not bound since it's tmux's prefix key and would never reach the program under tmux) — Up/Down are already claimed by history recall, so scrolling needed its own bindings regardless. Mouse-wheel scrolling is a Ctrl-T toggle (`m.mouseEnabled`, gating `tea.View.MouseMode`, since Bubble Tea v2 turns mouse reporting on per-View rather than as a `tea.NewProgram` option — the viewport's own `Update` already handles `tea.MouseWheelMsg` once it's on) rather than always-on: a terminal that requests mouse tracking is exactly what stops most terminals offering their own click-drag text selection, and the usual override (holding Shift while dragging) isn't reliable enough across every terminal to lean on by default. Off is the default — native selection and copy work out of the box — with the wheel a keystroke away for whoever wants it instead.

`/history`'s reprinted transcript goes through the same rendering `writeChatSession` now applies everywhere: each turn's answer is Markdown, and `writeChatSession(w, sess, renderAnswer)` takes a `renderAnswer func(string) string` so callers can supply their own policy — `ttyAnswerRenderer(w)` (glamour on a real terminal, passthrough otherwise, the same policy `renderMarkdown` uses, factored out as a string transform since `writeChatSession` interleaves the answer with the question and citations rather than handing a whole block to `renderMarkdown` at once) for `chat show` and the plain REPL, `chatModel.renderAnswer` (unconditional, viewport-width-bound) for the workspace. In the workspace specifically, `/history`'s whole captured block — glamour-rendered answers interleaved with plain question/citation text — is written `preWrapped` in one piece rather than split per-piece, the same as a single answer: refreshViewport's normal wrap pass would corrupt the rendered parts otherwise, and there's no cheap way to split a captured, already-interleaved text blob back into separately-wrappable pieces.

Typing a slash command gets live completion: `slashCommandPrefix` recognizes an in-progress command word (a line starting with `/` with no space yet — once a space appears the user has moved on to arguments, and completion stops applying), `matchingSlashCommands` filters `chatSlashCommands` (the single source of truth `chatHelp` itself is generated from) by that prefix, and a suggestions line — reserved as one constant line of height in `resize()`, so the layout doesn't jump on every keystroke — lists them below the viewport. Tab (`completeSlashCommand`) completes an unambiguous match to the full command plus a trailing space, or as far as every match's shared prefix goes when there's more than one (the same behavior a shell's Tab-completion has), and does nothing (falling through to the textarea's own Tab handling) when the input isn't a slash command at all.
