...
Scrolling the transcript is PgUp/PgDn, ctrl+u/ctrl+f (`ScrollUp`/`ScrollDown` in `chatKeyMap`, calling the viewport's own `PageUp`/`PageDown`; ctrl+b is deliberately not bound since it's tmux's prefix key and would never reach the program under tmux), or the mouse wheel (`tea.View.MouseMode = tea.MouseModeCellMotion`, since Bubble Tea v2 turns mouse reporting on per-View rather than as a `tea.NewProgram` option; the viewport's own `Update` already handles `tea.MouseWheelMsg`). Enabling mouse reporting does cost a plain click-drag its native text selection — most terminals stop offering that the moment an app requests mouse tracking — but nearly every terminal (xterm, iTerm2, kitty, Alacritty, GNOME Terminal/Konsole, Windows Terminal) still lets Shift+drag force a native selection regardless, which is the trade this leans on rather than dropping wheel-scroll. Up/Down are already claimed by history recall, so scrolling needed its own bindings regardless of the mouse question.
