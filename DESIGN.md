...
Scrolling the transcript is keyboard-only — PgUp/PgDn, or ctrl+u/ctrl+f (`ScrollUp`/`ScrollDown` in `chatKeyMap`, calling the viewport's own `PageUp`/`PageDown`); ctrl+b is deliberately not bound since it's tmux's prefix key and would never reach the program under tmux. The workspace does not enable mouse reporting (`tea.View.MouseMode` is left at its zero value): turning it on hands every mouse event to the program, which is exactly what stops a terminal from offering its own click-drag text selection and copy — losing copy/paste was judged worse than losing wheel-scroll, since PgUp/PgDn already cover scrolling. Up/Down are already claimed by history recall, so scrolling needed its own bindings regardless.
